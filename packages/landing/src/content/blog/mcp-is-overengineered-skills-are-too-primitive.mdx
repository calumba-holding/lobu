...
This matters a lot for sandboxing. In [Lobu](https://github.com/lobu-ai/lobu), workers run in containers with no direct internet access. They can't curl external APIs. They never see OAuth tokens. MCP servers sit behind the gateway proxy, which injects the right credentials per-user. The worker has no idea this is happening.
...
Over 100,000 packages on [Nixpkgs](https://search.nixos.org/packages). Every CLI tool you can think of. Installed declaratively, cached across container restarts, no Docker rebuild.
...
First boot pays the install cost. After that packages are cached on the workspace volume (Docker) or PVC (Kubernetes), so they survive container restarts and scale-to-zero.
