...
After `lobu init`, your project is configured through `lobu.toml`. Use the CLI to add providers and skills at any time:

```bash
# Browse and add providers
lobu providers list
lobu providers add gemini          # prompts for API key, updates lobu.toml + .env

# Browse and add skills
lobu skills list
lobu skills add github
lobu skills add linear

# Manage secrets
lobu secrets set GEMINI_API_KEY AIza...
lobu secrets list
...
Changes to `lobu.toml` take effect on next `lobu run` restart.
