...
- **`[memory.owletto]` enabled**: Lobu derives an Owletto MCP endpoint and uses `@lobu/openclaw-plugin`, falling back to native memory if the plugin is unavailable.
...
4. If memory is set to `@lobu/openclaw-plugin`, that plugin converts OpenClaw memory reads/writes into Owletto MCP calls through the gateway proxy.
